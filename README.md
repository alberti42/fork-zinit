...
| `extract`      | Performs archive extraction supporting multiple formats like `zip`, `tar.gz`, etc. and also notably OS X `dmg` images. If it has no value, then it works in the _auto_ mode – it automatically extracts all files of known archive extensions IF they aren't located deeper than in a sub-directory (this is to prevent extraction of some helper archive files, typically located somewhere deeper in the tree). If no such files will be found, then it extracts all found files of known **type** – the type is being read by the `file` Unix command. If not empty, then takes names of the files to extract. Supports prefix modifiers that control directory flattening after extraction: `extract'!'` flattens one directory level (useful when archives contain a single top-level directory like `tool-1.2.3/`), and `extract'!!'` flattens two directory levels. The `extract'-'` modifier prevents the archive file from being deleted after extraction. A specific filename can be given to extract only that file instead of auto-detecting archives (useful for cloned repos containing archives), e.g. `extract'!tool.tar.gz'` extracts and flattens only `tool.tar.gz`. Multiple space-separated filenames are supported. |
